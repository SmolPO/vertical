...
while True:
    print("___________")
    key = input("1 - поиск по ОЗУ, 2 - поиск по процессору, 3 - поиск по дате. Введите цифру: ")
    data = input("Введите значение для поиска: ")
    count = 0
    if key == "1":
        for i in range(len(list_comp)):
            if list_comp[i]['OZU'] == int(data):
                print(list_comp[i])
                count += 1
    elif key == "2":
        for i in range(len(list_comp)):
            if list_comp[i]['processor'] == data:
                print(list_comp[i])
                count += 1
    elif key == "3":
        for i in range(len(list_comp)):
            if list_comp[i]['start_market'] == data:
                print(list_comp[i])
                count += 1
    else:
        print("некорректный ввод")
    print("Найдено " + str(count) + " элементов")
