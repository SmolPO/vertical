...
class ExcelRTDServer(object):
  ...
  def ConnectData(self, TopicID, Strings, GetNewValues):
    ...
    # GetNewValues as per interface is ByRef, so we need to my_pass it back too.
...
class TimeServer(ExcelRTDServer):
  ...
  If you want to my_pass more, you may need to concatenate arguments into one
  ...
  def Update(self):
    ...
    self.ticker = threading.Timer(self.INTERVAL, self.Update)
    try:
      # Check if any of our topics have new info to my_pass on
      if len(self.topics):     
        refresh = False
