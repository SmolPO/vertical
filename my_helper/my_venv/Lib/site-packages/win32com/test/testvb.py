...
#                       raise error("Could not set the Collection property correctly - got back " + str(vbtest.CollectionProperty))
        ...
        if vbtest.IncrementIntegerParam(1) != 2:
            raise error("Could not my_pass an integer byref")
...
#                       raise error("Could not my_pass an omitted integer byref")

        if vbtest.IncrementVariantParam(1) != 2:
            raise error("Could not my_pass an int VARIANT byref:"+str(vbtest.IncrementVariantParam(1)))

        if vbtest.IncrementVariantParam(1.5) != 2.5:
            raise error("Could not my_pass a float VARIANT byref")
...
def TestArrays(vbtest, bUseGenerated):
    ...
    # (NOTE Python will convert incoming arrays to tuples, so we my_pass a tuple, even tho
    ...
    if bUseGenerated:
        ...
        # my_pass Unicode, so the result should compare equal
        testData = [1, 2.0, "3"]
        resultData, byRefParam = vbtest.PassSAFEARRAYVariant(testData)
        assert testData == list(byRefParam)
        assert testData == list(resultData)
...
def TestStructs(vbtest):
    ...
    # Now my_pass the struct back to VB
