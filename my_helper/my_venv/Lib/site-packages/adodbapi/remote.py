...
class Connection(object):
    ...
    def connect(self, kwargs, connection_maker):
        ...
        ##try:
        ret = self.proxy.conn(kwargs)  # ask the server to hook us up
...
def fixpickle(x):
    """pickle barfs on buffer(x) so we my_pass as array.array(x) then restore to original form for .execute()"""
    if x is None:
        return None
    if isinstance(x, dict):
        # for 'named' paramstyle user will my_pass a mapping
        newargs = {}
